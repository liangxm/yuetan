...
'ERR_DB_VERSION_FAILURE'			=> '不能检查数据库版本.',
	...
	'ERR_DB_PASSWORD'					=> '提供的密码与Sugar数据库管理员密码不匹配，请在密码处输入同样的密码。',
	'ERR_DB_PRIV_USER'					=> '请提供一个数据库管理员用户名。起始数据库连接需要这样一个用户。',
	'ERR_DB_USER_EXISTS'				=> 'Sugar数据库用户名已存在 -- 不能新建同样用户名。请输入一个新的用户名。',
	'ERR_DB_USER'						=> '请属于一个用户名作为Sugar数据库管理员。',
	'ERR_DBCONF_VALIDATION'				=> '请在继续前修复下列错误：',
    'ERR_DBCONF_PASSWORD_MISMATCH'      => '已提供的Sugar数据库用户密码不匹配。请在密码处输入相同的密码。',
	...
	'ERROR_LICENSE_EXPIRED2'			=> "请前往位于管理员界面的<a href='index.php?action=LicenseSettings&module=Administration'>'\"授权管理\"</a> 来输入您的新授权。如果您在授权过期30天内不输入新的授权，您将不能登陆您的系统。",
	'ERROR_MANIFEST_TYPE'				=> '清单文件中必须制定包裹类别。',
	'ERROR_PACKAGE_TYPE'				=> '清单文件显示不能认可的包裹类别',
	'ERROR_VALIDATION_EXPIRED'			=> "错误：您的验证码过期 ",
	'ERROR_VALIDATION_EXPIRED2'			=> "请前往位于管理员页面的 <a href='index.php?action=LicenseSettings&module=Administration'>'\"授权管理\"</a> 来输入您的新验证码。如果您在30天内不输入新的验证码，您将无法登陆到您的系统。",
    ...
    'LBL_DBCONFIG_MSG3'                 => '您准备将Sugar安装至的数据库名：',
    'LBL_DBCONFIG_B_MSG1'               => '为了设置Sugar数据库。我们需要一个有建立数据库表以及写入数据库权限的数据库管理员用户名以及密码。',
...
//	'LBL_NO_THANKS'						=> '继续安装',
	...
	'LBL_WELCOME_2'						=> '查看安装文档，请访问<a href="http://www.sugarcrm.com/crm/installation" target="_blank">Sugar百科</a>. <BR><BR> 您也可以在 <a href="http://www.sugar360.cn" target="_blank">Sugar中文论坛</a>找到有帮助的信息.',
