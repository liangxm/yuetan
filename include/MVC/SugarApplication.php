...
class SugarApplication
{
		...
		if(!empty($_SESSION['authenticated_user_id'])){
			if(isset($_SESSION['hasExpiredPassword']) && $_SESSION['hasExpiredPassword'] == '1'){
			    ...
			    if(empty($ut)
					...
					//$record = $GLOBALS['current_user']->id;
					$GLOBALS['current_user']->setPreference('timezone', 'Asia/Shanghai');
					$GLOBALS['current_user']->setPreference('ut', 1);
					$GLOBALS['current_user']->savePreferencesToDB();
					session_write_close();
					   header('Location: index.php?action=index&module=lxm1_lxm_residents');
				    exit();
				}else{
	...
	*/
	...
	public function getLoginRedirect()
	{
        ...
        if(empty($vars)) {
            return "index.php?module=lxm1_lxm_residents&action=index";
        } else {
